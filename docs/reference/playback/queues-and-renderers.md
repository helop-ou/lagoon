...
- **Profile 7 conversion**: profile 7 (`DOVIWithEL`/`DOVIWithELHDR10Plus`)
  ...
  (`Lagoon/Features/Playback/Engine/`) converts it live instead: every type-62
  ...
  hashes are in `Packages/LagoonFFmpeg/Artifacts/Libdovi.README.md`.
  ...
  **Compatibility fallback**: Settings → Advanced → Playback Diagnostics →
  ...
  behavior before profile 7 conversion existed. The setting itself began as an
  experiment called "Strip DoVi Enhancement Layer", since retired.
...
- **`A 0` is not a demux refill failure.** The report was first dismissed as
  ...
  every direct-played MKV is. A Jellyfin HLS fMP4 segment is not: each
  ...
  **The fix: a compressed video intake in front of every decoder.**
  ...
  regression probe, beside a `rung=` field that names the delivery rung
...
- **A stream with no cache gets a bigger demux cushion.** The sparse AVIO
  ...
  because a Jellyfin HLS transcode has mutable manifests and a byte-range
...
- **Audio delay** (M6): mpv convention, positive delays audio; applied by
  ...
  re-demuxing from the current position on change. Lives in the Audio tab's
  OPTIONS column.
