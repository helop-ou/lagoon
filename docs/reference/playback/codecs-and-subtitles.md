...
- **Deinterlacing.** The software decode path makes an interlaced frame
  ...
  also what makes a DVD image playable at all: with it in place, Jellyfin
...
- **Anamorphic and non-square pixels.** `SampleBufferFactory` attaches
  ...
  of rounding artifacts: of the 245 items Jellyfin flags `IsAnamorphic` on the
  fixture server, 203 have a genuine pixel aspect — 16:15 and 64:45 PAL, 4:3
...
- **Subtitles.** Rendered as a SwiftUI overlay, never through the renderers.
  ...
  on the codec's graphics plane. External Jellyfin streams, delivered as vtt,
...
- **Subtitle download safety.** External sidecars and Jellyfin provider files
...
- **One subtitle source.** Jellyfin's remote-subtitle routes require the
  account's `EnableSubtitleManagement` permission, which is off by default for
  every non-administrator — the common case on a shared server. Administrators
  pass regardless of the stored flag, and an absent value is let through
  rather than treated as a denial; either way the server's own `403` is what
  decides. When the permission holds, Lagoon searches preferred languages
  concurrently, fetches the matched provider file directly for immediate
  playback — side-loading and selecting it without restarting the video — and
  uploads those same bytes to Jellyfin so the sidecar persists for every
  client and viewer. Provider formats Lagoon cannot parse fall back to
  Jellyfin's native save plus a PlaybackInfo poll for the new stream;
  oversized or HTML responses trigger neither that fallback nor an automatic
  retry. Lagoon used to run a direct OpenSubtitles provider for accounts
  without the permission, but removed it: OpenSubtitles' REST terms require
  one API key per application and ban apps that ask users to supply their own,
  which is what the per-device-key design did. Settings shows the account's
  permission state so the viewer knows whether to ask their server
  administrator.
- **Text encoding.** `SubtitleTextDecoder` replaces a fallback chain that
  ...
  but a file fetched through Jellyfin's `Providers/Subtitles/Subtitles/{id}`
