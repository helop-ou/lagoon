...
- The current player controls, auto-hide, and native options-sheet behavior
  are specified in [Playback](../playback.md#controls-and-presentation).
  Keep adjustable VoiceOver seeking and the separate tvOS remote grammar.
...
the same 16 MiB per-response cap. See [download hardening validation](../archive/download-hardening-validation.md).
