...
September 10, 2026 documentation cleanup. Start with the [current playback guide](../playback.md).
...
| Topic | Notes |
| --- | --- |
| Input and negotiation | [Transport](#network-transport), [stream resolution](#stream-resolution), [disc images](#disc-images-hel-133) |
| Decode and performance | [Engine](#the-engine-lagoonviewsplayersamplebuffer), [codec/timing details](#codec-and-timing-details), [panel performance](#player-panel-performance) |
| Subtitles and rendering | [Subtitles](#subtitles), [queues](#queue-and-renderer-behavior), [system media](#system-media-integration-hel-41-hel-80) |
| Measurement and ownership | [Frame-loss bench](#frame-loss-bench-hel-64), [memory](#decoded-frame-memory-ceiling-hel-109), [cache/teardown](#playback-cache-and-teardown) |
| UI and server state | [Progress](#progress-reporting), [iOS controls](#ios-player-controls), [remote reveal](#siri-remote-transport-reveal), [tvOS gotchas](#player-view-gotchas-learned-the-hard-way-on-tvos) |
...
The [HEL-142 validation record](../archive/hel-142-native-tls-validation.md) documents
...
- **Subtitle download safety** (audit A15): external sidecars and Jellyfin
  ...
  failures pause or restart playback. See the [validation record](../archive/download-hardening-validation.md).
...
The cache history below predates the HEL-142 transport replacement. References
to native HTTP or native HLS I/O describe the former implementation. Current
cache misses and uncached playback both use the URLSession transport described
in [Network transport](../playback.md#network-transport).
...
The current touch layout, auto-hide, audio policy, and PiP ownership are
specified in the [playback guide](../playback.md#controls-and-presentation).
The [HEL-153 record](../archive/hel-153-touch-validation.md) holds simulator
results and the remaining physical-device checks.
