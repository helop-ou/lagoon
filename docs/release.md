...
resolved, and records the engine version and revision it read; the script
refuses a checkout that is not the one `Package.resolved` pins. Keep exact
...
- [ ] Deliver and test the corresponding-source, notices and relinking
  ...
  most cheaply and removes the per-release burden. Since engine 1.0.2 all four
  FFmpeg libraries are built by the engine from checksum-pinned source without
  `--enable-version3`, and are LGPL-2.1-or-later; dav1d, lcms2 and uavs3d are
  built there too. libdovi is the one component vendored prebuilt, because it
  needs a Rust toolchain. Whether any route satisfies the licence for App
  Store distribution is a legal judgement, not an engineering one.
