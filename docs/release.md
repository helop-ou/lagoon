...
**Caution.** That assessment is a snapshot from one point in time. Reconcile
it against the actual release before making distribution declarations.

## Public release

These gates remain separate from the internal TestFlight procedure.

**Caution.** Check them against the final signed candidate, not an earlier
audit revision.
