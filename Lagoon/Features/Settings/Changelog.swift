...
nonisolated enum Changelog {
    ...
    static let entries: [ChangelogEntry] = [
        ChangelogEntry(
            ...
            changes: [
                "Films and episodes can be downloaded to an iPhone or iPad and watched without the server. The download button on a title's page offers Original, High (1080p) and Standard (720p) with an estimated size, transfers continue while the app is in the background, and downloaded titles carry a mark on their posters. Your downloads are listed at the top of Library and keep working when the server cannot be reached, and where you stop watching reaches the server once it is back. Settings has a Downloads page for the default quality, Wi-Fi only and storage. The account needs the server's Allow media downloading permission.",
                "Film and series pages on iPhone and iPad are redesigned. The poster fills the top of the page in portrait and is shown whole over a soft blur of itself in landscape, a large Play button sits under the title with the watched, favorite and From Beginning buttons beneath it as a row of circles, and the whole synopsis is shown instead of a Read Synopsis button. On an iPad in a full-width window the page keeps its landscape artwork and side-by-side layout, with more of the artwork showing. The tab bar steps out of the way while a film or series page is open.",
